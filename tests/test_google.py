...
_ = pytest.__version__  # make sure to use pytest
...
def test_google_import_spreadsheet():
    ...
    google_data_noheaders = google_api.import_spreadsheet(
        TEST_SHEET_ID, IMPORT_RANGE, column_header=False, row_index=False
    )
    google_data_headers = google_api.import_spreadsheet(
        TEST_SHEET_ID, IMPORT_RANGE, column_header=True, row_index=True
    )
